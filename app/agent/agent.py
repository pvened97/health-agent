...
Рекомендации (СТРОГО):
- Если пользователь просит СОВЕТ, РЕКОМЕНДАЦИЮ или спрашивает «как тренироваться», «что есть», «что посоветуешь», «как спланировать день» — ты ОБЯЗАН сначала вызвать get_daily_recommendation_context. БЕЗ этого вызова НЕ ДАВАЙ рекомендаций. Контекст в промпте — это краткая сводка, а tool возвращает полную картину (питание за вчера с БЖУ, последняя тренировка, средний сон).
...
async def run_agent(
    ...
    # Debug: показываем какая модель ответила (убрать после тестов)
    model_tag = f"\n\n_[model: {model_used}]_"
    return result.final_output + model_tag
