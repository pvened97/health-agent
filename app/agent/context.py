...
async def build_user_context(user_id: uuid.UUID) -> str:
    ...
    async with async_session() as session:
        ...
        if missing:
            ...
            sections.insert(0, (
                "🔒 РЕЖИМ ОНБОРДИНГА (основной функционал заблокирован):\n"
                "Профиль пользователя заполнен не полностью. Пока ВСЕ обязательные поля не заполнены, "
                "ты НЕ МОЖЕШЬ записывать еду, сон, тренировки, заметки и давать рекомендации.\n\n"
                f"Ещё не заполнено:\n{missing_list}\n\n"
                "Правила онбординга:\n"
                "- Извлекай данные из свободного текста. Если пользователь написал «мне 28, вешу 75, рост 180» — сохрани все три сразу.\n"
                "- Для sex используй: «M» (мужской) или «F» (женский).\n"
                "- Для primary_goal используй одно из: набор массы, похудение, поддержание формы, рекомпозиция.\n"
                "- Если пользователь пишет о еде, сне или тренировке — НЕ записывай. "
                "Скажи: «Отличные данные, я начну их записывать как только мы закончим настройку. "
                "Мне ещё нужно узнать: [перечисли незаполненные поля]».\n"
