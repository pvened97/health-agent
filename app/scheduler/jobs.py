...
async def weekly_summary(bot):
    ...
    for user, tg_account in rows:
        ...
        try:
            response = await run_agent(
                "Дай подробный обзор за эту неделю: сон, питание, тренировки, recovery. "
                "По питанию — покажи среднее за неделю и сравни с целями из профиля "
                "(калории и белок: факт vs цель, процент). Сравни с прошлой неделей. Что улучшить?",
                user_id=user.id,
                trigger="scheduler",
            )
            from app.telegram.handlers import _md_to_html
